...
units = dict([s, [u for (u of unitlist) if (u.contains(s))]] for (s of squares))
peers = dict([s, set([s2 for (u of units[s]) for (s2 of u) if (s2 != s)])] for (s of squares))
...
function print_board (values) {
  ...
  for (let r in rows) {
    print([values[r+c].center(width) + ('36'.contains(c) && '|' || '') for (c in cols)].join('') + ('CF'.contains(r) && line || ''))
  }
  print()
}
